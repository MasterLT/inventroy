...
public class ExcelUtil {
     ...
     * @param time        盘点时间
     */
    public static void exportExcelX(String title, Map<String, String> headMap, JSONArray jsonArray, String datePattern, int colWidth, OutputStream out, String time) {
        if (datePattern == null) datePattern = DEFAULT_DATE_PATTERN;
        // 声明一个工作薄
        ...
        titleStyle.setFont(titleFont);
        // 列头样式
        CellStyle headerStyle = getHeadStyle(workbook);
        // 单元格样式
        CellStyle cellStyle = getCellStyle(workbook);
        // 底部样式
        CellStyle bottomStyle = getBottomStyle(workbook);
        // 生成一个(带标题)表格
        SXSSFSheet sheet = workbook.createSheet();
        //设置列宽
        int minBytes = colWidth < DEFAULT_COLOUMN_WIDTH ? DEFAULT_COLOUMN_WIDTH : colWidth;//至少字节数
        int[] arrColWidth = {6, 11, 14, 14, 6, 10, 10, 8, 10};
        // 产生表格标题行,以及设置列宽
        String[] properties = new String[headMap.size()];
        String[] headers = new String[headMap.size()];
        int ii = 0;
...
//        sheet.autoSizeColumn(1, true);
        ...
        for (Object obj : jsonArray) {
            if (rowIndex == 65535 || rowIndex == 0) {
                if (rowIndex != 0) sheet = workbook.createSheet();//如果数据超过了，则在第二页显示
                //表头 rowIndex=0
                createCell(sheet, title, rowIndex++, headMap.size(), titleStyle);
                createCell(sheet, "", rowIndex++, headMap.size(), getHeadStyle2(workbook));
                //盘点时间
                createCell(sheet, "盘点时间：" + time + "                    ", rowIndex++, headMap.size(), getHeadStyle2(workbook));
                createCell(sheet, "", rowIndex++, headMap.size(), getHeadStyle2(workbook));
                //列头
        ...
        //底部领导签字
        createCell(sheet, "", rowIndex++, headMap.size(), bottomStyle);
        createCell(sheet, "盘  点  人  ：", rowIndex++, headMap.size(), bottomStyle);
        createCell(sheet, "", rowIndex++, headMap.size(), bottomStyle);
        createCell(sheet, "部  门  经  理  ：", rowIndex++, headMap.size(), bottomStyle);
        createCell(sheet, "", rowIndex++, headMap.size(), bottomStyle);
    ...
    private static void createCell(SXSSFSheet sheet, String value, Integer rowIndex, Integer size, CellStyle style) {
        SXSSFRow titleRow = sheet.createRow(rowIndex);
        titleRow.createCell(0).setCellValue(value);
        titleRow.getCell(0).setCellStyle(style);
        sheet.addMergedRegion(new CellRangeAddress(rowIndex, rowIndex, 0, size - 1));
    }
    ...
    private static CellStyle getHeadStyle2(SXSSFWorkbook workbook) {
        CellStyle headerStyle = workbook.createCellStyle();
        headerStyle.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
        headerStyle.setFillForegroundColor(HSSFColor.WHITE.index);
    ...
    private static CellStyle getBottomStyle(SXSSFWorkbook workbook) {
        ...
        bottomStyle.setAlignment(HSSFCellStyle.ALIGN_LEFT);
        Font headerFont = workbook.createFont();
        headerFont.setFontHeightInPoints((short) 16);
        headerFont.setBoldweight(HSSFFont.BOLDWEIGHT_NORMAL);
        bottomStyle.setFont(headerFont);
        return bottomStyle;
    }
    ...
    //Web 导出excel
    public static void downloadExcelFile(String title, Map<String, String> headMap, JSONArray ja, HttpServletResponse response, String time) {
        try {
            ByteArrayOutputStream os = new ByteArrayOutputStream();
            ExcelUtil.exportExcelX(title, headMap, ja, null, 0, os, time);
    ...
    public static void main(String[] args) throws IOException {
        ...
        Date d2 = new Date();
        ExcelUtil.exportExcelX(title, headMap, ja, null, 0, outXlsx, "ddd");
